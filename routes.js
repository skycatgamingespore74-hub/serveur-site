...
router.post('/register', (req, res) => {
    ...
    const newUser = { email, password, telephone: telephone || '', credits: 0, createdAt: new Date() };
    users.push(newUser);
    saveUsers(users);
...
router.post('/login', (req, res) => {
    ...
    if (!user) { addLog(`Échec connexion : ${email}`); return res.status(400).json({ error: 'Email ou mot de passe incorrect' }); }
...
router.post('/check-session', (req, res) => {
    ...
    res.json({ success: !!email, email });
});

// LOGOUT
router.post('/logout', (req, res) => {
    const { token } = req.body;
    if (tokens[token]) { addLog(`Déconnexion : ${tokens[token]}`); delete tokens[token]; }
...
router.post('/credits/:email', (req, res) => {
    ...
    if (!amount || amount <= 0 || isNaN(amount)) return res.status(400).json({ error: 'Montant invalide' });
...
router.get('/admin/status', (req, res) => {
    res.json({ connected: true, usersCount: getUsers().length, time: new Date().toISOString() });
});

router.get('/admin/users', (req, res) => res.json({ success: true, users: getUsers() }));
router.get('/admin/logs', (req, res) => res.json({ success: true, logs: getLogs() }));
